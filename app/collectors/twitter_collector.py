                    ...
                    for entry in feed.entries[:10]:  # Check more entries to find recent ones
                        ...
                        if hasattr(entry, "published_parsed") and entry.published_parsed:
                            ...
                            if diff.total_seconds() > 24 * 3600:
                                logger.debug(
                                    f"Skipping old tweet from {handle} ({diff.total_seconds() / 3600:.1f}h ago)"
                                )
                                continue
                        else:
                            continue
                        ...
                        if hasattr(entry, "enclosures"):
                            for enclosure in entry.enclosures:
                                if enclosure.type.startswith("video") or enclosure.href.endswith(
                                    (".mp4", ".m4v", ".webm")
                                ):
                        ...
                        if not video_url:
                            for source in soup.find_all("source"):
                                if source.get("src") and any(
                                    source.get("src").endswith(ext) for ext in [".mp4", ".m4v", ".webm"]
                                ):
                        ...
                        collected.append(
                            {
                                ...
                                "video": tweet_url
                                if video_detected
                                else None,  # Use tweet URL as the video source for yt-dlp
                                "url": tweet_url,
